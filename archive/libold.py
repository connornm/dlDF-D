...
#########################################################
#	Input:
#	systems - array of systems to get total error
#	from
#	eSAPT - Provide the ebench energies/ variables,
#	if not arguement then read them from file 
#	dispE - Dicionary of dispersion energies to add
#	to total input energy
#########################################################
#	Output:
#	error - total error
#########################################################
def get_error(systems, eSAPT = {}, eDISP = {}):
	if (eSAPT == {}):
		SAPT_data = read_bench_mult(systems)
		for system in systems:
			eSAPT[system] = SAPT_data[system]['E']
	else:
		pass
	if (eDISP == {}):
		eDISP = mult_disp(systems)
	else:
		pass
	eDFT = mon_dim_energy(systems)
	error = 0
	for system in systems:
		if (eSAPT[system] == 0):
			pass
		else:
			# Need conversion factor here
			dE = (eDFT[system+'_D']-eDFT[system+'_A']-eDFT[system+'_B'])*627.51
			dE += eDISP[system] 
			error += ((eSAPT[system]-dE)/eSAPT[system])**2
	return error
